...
function __aeMakeLRU(maxSize) {
  ...
  return {
    get: function(k) {
      ...
      let v = cache.get(k); cache.delete(k); cache.set(k, v);
      return v;
    },
    set: function(k, v) {
...
const __AE_SEARCH_CACHE = __aeMakeLRU(50); // per-page cache for automatic web-search contexts with LRU(50) limit
...
function __aeLoadSettings() {
  try {
    let stored = localStorage.getItem(__AE_STORAGE_KEY);
...
// --- CDN Library Loaders ---
const __aePromises = Object.create(null);
...
async function __aeLoadPdfJs() {
  return (__aePromises.pdfjs ??= (async function() {
    console.log('[ae] Loading pdf.js...');
    let loadedUrl = await __aeLoadScriptFromAny([
...
async function __aeLoadMammoth() {
  return (__aePromises.mammoth ??= (async function() {
...
async function __aeLoadXlsx() {
  return (__aePromises.xlsx ??= (async function() {
...
async function __aeLoadJsZip() {
  return (__aePromises.jszip ??= (async function() {
...
async function __aeLoadTransformers() {
  return (__aePromises.transformers ??= (async function() {
    console.log('[ae] Loading Transformers.js...');
    let mod = await import('https://cdn.jsdelivr.net/npm/@xenova/transformers@2.17.2');
    try {
      if (mod.env) {
...
function __aeChunkText(text, wordsPerChunk) {
  ...
  let words = text.split(/\s+/).filter(function(w) { return w.length > 0; });
...
async function __aeAddLoreEntry(text, sourceLabel) {
  ...
  let thread = await db.threads.get(tid);
  if (!thread) {
    console.warn('[ae] Thread not found:', tid);
    return 0;
  }
  let loreBookId = thread.loreBookId != null ? thread.loreBookId : tid;
  let modelName = thread.textEmbeddingModelName || 'Xenova/bge-base-en-v1.5';
  ...
  for (let ei = 0; ei < existingEntries.length; ei++) {
  ...
  let embedding = null;
  try {
    let embeddings = await embedTexts({ textArr: [text], modelName: modelName, shouldCache: true });
...
async function __aeAddSystemMessage(content, name) {
  ...
  let id = await addMessageToDb(msgObj);
  // addMessageToDb clones before saving, so the original object does not receive
  // the auto-increment id. Without this, freshly-added system messages render with
  // an undefined data-id and cannot be deleted reliably from the feed.
  msgObj.id = id;
...
function __aeBuildAiContextContent(kind, fileName, text, meta) {
  text = (text || '').trim();
  meta = meta || '';
  let truncated = text.length > __AE_CONTEXT_PREVIEW_CHARS;
  let preview = truncated ? text.slice(0, __AE_CONTEXT_PREVIEW_CHARS) + '\n\n[... content truncated; full text was stored in lore chunks when possible ...]' : text;
  return [
...
async function __aeAddAiContextMessage(content, name, customData) {
  ...
  let id = await addMessageToDb(msgObj, {doNotReRenderThreadList:true});
  return id;
}
...
async function __aeLoadFileRegistry() {
  let obj = await db.misc.get(__AE_FILE_REGISTRY_KEY).catch(function() { return null; });
...
async function __aeRegisterUploadedFile(meta) {
  let registry = await __aeLoadFileRegistry();
...
async function __aeUpdateUploadedFile(id, patch) {
  let registry = await __aeLoadFileRegistry();
  let file = registry.files.find(function(f) { return f.id === id; });
  if (!file) return null;
...
async function __aeGetUploadedFiles(opts) {
  opts = opts || {};
  let registry = await __aeLoadFileRegistry();
  let files = registry.files.slice();
...
async function __aeFindUploadedFileByMention(name, threadId) {
  let wanted = __aeNormalizeFileName(name);
  let files = await __aeGetUploadedFiles({threadId: threadId});
  let exact = files.filter(function(f) { return f.normalizedName === wanted || __aeNormalizeFileName(f.name) === wanted; });
  if (exact.length === 1) return { file: exact[0], matches: exact };
  if (exact.length > 1) return { file: null, matches: exact };
  let fuzzy = files.filter(function(f) {
    let n = f.normalizedName || __aeNormalizeFileName(f.name);
...
async function __aeSetContextMessageActive(messageId, active) {
  ...
  let msg = await db.messages.get(messageId).catch(function() { return null; });
  if (!msg) return;
  let hiddenFrom = Array.isArray(msg.hiddenFrom) ? msg.hiddenFrom.slice() : [];
...
async function __aeEnforceFileContextBuffer(threadId) {
  let registry = await __aeLoadFileRegistry();
  let files = registry.files.filter(function(f) {
    return f.threadId === threadId && f.contextActive !== false && typeof f.contextMessageId === 'number';
  }).sort(function(a, b) { return (a.uploadedAt || 0) - (b.uploadedAt || 0); });
  let total = files.reduce(function(sum, f) { return sum + (f.contextCharCount || 0); }, 0);
  let changed = false;
...
async function __aeReactivateFileContext(file) {
  ...
  if (file.fullText) {
    let content = __aeBuildAiContextContent(file.kind || 'File', file.name, file.fullText, file.metaText || 'reactivated mention');
    let messageId = await __aeAddAiContextMessage(content, 'File Mention', { __aeFileContext: true, fileId: file.id, source: 'mention' });
...
async function __aeRememberExtractedContent(kind, fileName, text, meta) {
  ...
  if (text && text.trim()) {
    __aeAssertNotCancelled();
    let currentContextChars = await __aeGetActiveFileContextCharCount(activeThreadId);
    ...
    let content = __aeBuildAiContextContent(kind, fileName, text, meta);
    let fileRecord = await __aeRegisterUploadedFile({
      threadId: activeThreadId,
      name: fileName,
      kind: kind,
      metaText: meta || '',
    ...
    let messageId = await __aeAddAiContextMessage(content, kind + ' Upload', { __aeFileContext: true, fileId: fileRecord.id, fileName: fileName, kind: kind });
...
function __aeRenderTaskPanel() {
  ...
  let tasks = Array.from(__AE_PROCESSING_TASKS.values());
  if (tasks.length === 0) {
    __aeTaskPanelEl.remove();
    __aeTaskPanelEl = null;
    return;
  }
  let html = '<div style="display:flex;justify-content:space-between;gap:0.75rem;align-items:center;margin-bottom:0.35rem;"><b>📎 Processing files</b><button id="__aeCancelTasksBtn" style="font-size:12px;">cancel all</button></div>';
  html += tasks.map(function(t) {
    let icon = t.status === 'done' ? '✅' : t.status === 'error' ? '❌' : t.status === 'cancelled' ? '🛑' : '⏳';
...
function __aeUpdateTask(id, detail, status) {
  let task = __AE_PROCESSING_TASKS.get(id);
  if (!task) return;
...
function __aeEndTask(id, status, detail) {
  ...
  __aeRenderTaskPanel();
  setTimeout(function() {
    let t = __AE_PROCESSING_TASKS.get(id);
...
let __aeOriginalGetBotReply = getBotReply;
getBotReply = async function(opts) {
  let nextOpts = opts;
  for (let i = 0; i < __AE_BEFORE_BOT_REPLY_HOOKS.length; i++) {
    let hook = __AE_BEFORE_BOT_REPLY_HOOKS[i];
    try {
      let result = await hook.fn(nextOpts);
