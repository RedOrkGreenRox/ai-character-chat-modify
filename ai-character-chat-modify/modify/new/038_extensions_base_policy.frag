...
function __aeGetLanguagePreset(codeOrId) {
  ...
  let packs = __aeGetAvailableLanguagePacks();
  let found = packs.find(function(l) { return l.code === codeOrId || l.id === codeOrId; });
...
function __aeNormalizeBasePolicy(policy) {
  ...
  if (Array.isArray(policy.allowedLanguagePacks)) {
    allowed = policy.allowedLanguagePacks.map(function(x) {
      let p = byId[x] || byCode[x];
      return p ? p.id : null;
    }).filter(Boolean);
  } else if (policy.language) {
    if (policy.language === 'off') allowed = [];
    else {
      let p = byCode[policy.language] || byId[policy.language];
  ...
  let primary = policy.primaryLanguagePack || policy.fallbackLanguagePack || policy.fallbackLanguage || 'accm.lang.en';
  let primaryPack = byId[primary] || byCode[primary] || byId['accm.lang.en'] || packs[0];
  primary = primaryPack ? primaryPack.id : '';
  if (allowed.length > 0 && allowed.indexOf(primary) === -1) primary = allowed[0];

  let firstPack = allowed.length ? byId[allowed[0]] : null;
...
async function __aeGetThreadBasePolicy(threadId) {
  ...
  let policy = thread && thread.customData && thread.customData.__aeBasePolicy;
...
async function __aeSetThreadBasePolicy(threadId, policy) {
  ...
  let customData = Object.assign({}, thread.customData || {});
  let current = __aeNormalizeBasePolicy(customData.__aeBasePolicy || null);
...
function __aeBuildBasePolicyPrompt(policy) {
  ...
  let allowed = (policy.allowedLanguagePacks || []).map(function(id) { return byId[id]; }).filter(Boolean);
  if (allowed.length === 0) return '';
  let primary = byId[policy.primaryLanguagePack] || byId[policy.fallbackLanguagePack] || allowed[0];
  let allowedNames = allowed.map(function(p) { return p.name; }).join(', ');
...
__aeRegisterBeforeBotReplyHook('basePolicy', async function(opts) {
  ...
  let policy = await __aeGetThreadBasePolicy(opts.threadId);
  let prompt = __aeBuildBasePolicyPrompt(policy);
  if (!prompt) return opts;

  let newMessages = opts.messages.slice();
  let policyMsg = __aeCreateTransientMessageObj({
...
async function __aeShowBasePolicyModal() {
  ...
  let current = await __aeGetThreadBasePolicy(activeThreadId);
  let packs = __aeGetAvailableLanguagePacks();
  ...
  function render() {
    ...
    async function savePolicyAutomatically() {
      let allowedIds = Array.from(win.bodyEl.querySelectorAll('.__aePolicyLangToggle')).filter(function(x) { return x.checked; }).map(function(x) { return x.dataset.id; });
...
async function __aeShowBasePolicyStatus() {
  let policy = await __aeGetThreadBasePolicy(activeThreadId);
  let packs = __aeGetAvailableLanguagePacks();
  let names = (policy.allowedLanguagePacks || []).map(function(id) { return packs.find(function(p) { return p.id === id; }); }).filter(Boolean).map(function(p) { return p.label; });
  let primary = packs.find(function(p) { return p.id === (policy.primaryLanguagePack || policy.fallbackLanguagePack); });
...
if (window.__accm && window.__accm.commands) {
  ...
  window.__accm.commands.register({
    ...
    handler: async function(text) {
      let code = String(text || '').trim().slice('/language '.length).trim().toLowerCase();
      if (code === 'off') {
      ...
      let lang = __aeGetLanguagePreset(code);
