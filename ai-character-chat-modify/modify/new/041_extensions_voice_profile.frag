
// ============================================
// AI CHARACTER CHAT — VOICE PROFILE MODULE
// ============================================
// Lightweight acoustic profile for uploaded/recorded audio.
// Does NOT identify a person; only describes audible qualities with uncertainty.
// ============================================

function __aeEstimatePitchAutocorrelation(samples, sampleRate) {
  if (!samples || samples.length < sampleRate * 0.1) return null;
  let size = Math.min(samples.length, sampleRate * 2);
  let start = Math.max(0, Math.floor((samples.length - size) / 2));
  let x = samples.slice(start, start + size);
  let rms = Math.sqrt(x.reduce(function(sum, v) { return sum + v * v; }, 0) / x.length);
  if (rms < 0.01) return null;
  let minLag = Math.floor(sampleRate / 350);
  let maxLag = Math.floor(sampleRate / 70);
  let bestLag = 0, best = -Infinity;
  for (let lag = minLag; lag <= maxLag; lag++) {
    let corr = 0;
    for (let i = 0; i < x.length - lag; i += 3) corr += x[i] * x[i + lag];
    if (corr > best) { best = corr; bestLag = lag; }
  }
  return bestLag ? sampleRate / bestLag : null;
}

async function __aeAnalyzeAudioProfile(arrayBuffer, transcript) {
  let AudioCtx = window.AudioContext || window.webkitAudioContext;
  if (!AudioCtx) return '';
  try {
    let ctx = new AudioCtx({ sampleRate: 16000 });
    let decoded = await ctx.decodeAudioData(arrayBuffer.slice(0));
    let sr = decoded.sampleRate;
    let ch = decoded.numberOfChannels;
    let len = decoded.length;
    let mono = new Float32Array(len);
    for (let c = 0; c < ch; c++) {
      let data = decoded.getChannelData(c);
      for (let i = 0; i < len; i++) mono[i] += data[i] / ch;
    }
    try { await ctx.close(); } catch(e) {}

    let duration = len / sr;
    let rms = Math.sqrt(mono.reduce(function(sum, v) { return sum + v * v; }, 0) / Math.max(1, mono.length));
    let zc = 0;
    for (let zi = 1; zi < mono.length; zi += 4) if ((mono[zi - 1] < 0 && mono[zi] >= 0) || (mono[zi - 1] >= 0 && mono[zi] < 0)) zc++;
    let zcr = zc / Math.max(1, mono.length / 4);
    let pitch = __aeEstimatePitchAutocorrelation(mono, sr);
    let words = (transcript || '').split(/\s+/).filter(Boolean).length;
    let wpm = duration > 0 ? Math.round(words / duration * 60) : 0;

    let pitchLabel = !pitch ? 'uncertain' : pitch < 120 ? 'low' : pitch < 190 ? 'medium' : 'high';
    let volumeLabel = rms < 0.025 ? 'quiet' : rms < 0.09 ? 'moderate' : 'loud';
    let paceLabel = !wpm ? 'uncertain' : wpm < 100 ? 'slow' : wpm < 170 ? 'normal' : 'fast';
    let toneHints = [];
    if (rms > 0.11 || zcr > 0.12) toneHints.push('energetic/tense possible');
    if (rms < 0.03) toneHints.push('soft/quiet');
    if (wpm > 180) toneHints.push('hurried');
    if (wpm > 0 && wpm < 90) toneHints.push('calm/slow');

    return [
      'VOICE ACOUSTIC PROFILE (approximate, not identity):',
      '- duration: ' + duration.toFixed(1) + 's',
      '- loudness: ' + volumeLabel,
      '- pitch impression: ' + pitchLabel + (pitch ? ' (~' + Math.round(pitch) + ' Hz)' : ''),
      '- speaking pace: ' + paceLabel + (wpm ? ' (~' + wpm + ' wpm)' : ''),
      '- tone hints: ' + (toneHints.length ? toneHints.join(', ') : 'neutral/uncertain'),
      '- perceived age/gender: not reliably inferred; avoid treating this as fact.'
    ].join('\n');
  } catch(e) {
    console.warn('[ae] voice acoustic profile failed:', e);
    return '';
  }
}

console.log('[ae] Voice Profile module loaded. Use /toggle voiceprofile.');
