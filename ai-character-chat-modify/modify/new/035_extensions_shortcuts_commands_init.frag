...
async function __aeShowExtensionsMenu() {
  ...
  async function render() {
    ...
    win.bodyEl.querySelector('.__aeMenuSearch')?.addEventListener('click', function() {
      let q = win.bodyEl.querySelector('.__aeMenuSearchInput').value.trim();
...
async function __aeEnsureShortcutButtons(thread) {
  ...
  // Extension/global navigation now lives in the ACCM sidebar, not in per-thread shortcut buttons.
  // Keep this normalizer so old extension buttons are removed from existing threads.
...
let __aeOriginalSendButtonClickHandler = sendButtonClickHandler;
sendButtonClickHandler = async function() {
  let text = ($.messageInput.value || '').trim();
  if (text && text[0] === '/') {
    try {
      let handled = await __aeHandleCommandText(text);
      if (handled) {
        $.messageInput.value = '';
        return;
      }
...
let __aeOriginalShowThread = showThread;
...
function __aeUpdateInputTooltip() {
  ...
  let existing = $.messageInput.getAttribute('title') || '';
...
function __aeStyleShortcutButtons() {
  try {
    let buttons = $.shortcutButtonsCtn ? $.shortcutButtonsCtn.querySelectorAll('button') : [];
...
let __aeCommandSuggestEl = null;
let __aeCommandSuggestItems = [];
let __aeCommandSuggestIndex = 0;
...
function __aeRankCommandSuggestion(input, item) {
  ...
  let words = q.replace(/^\//,'').split(/\s+/).filter(Boolean);
  let score = 0;
...
function __aeUpdateCommandSuggestions() {
  ...
  let scored = __AE_COMMAND_SUGGESTIONS.map(function(item) {
  ...
  let el = __aeEnsureCommandSuggestEl();
  let rect = $.messageInput.getBoundingClientRect();
